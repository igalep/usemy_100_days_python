...
class VaultAccess:
    ...
    def __init__(self):
        ...
        if not hasattr(self, '_initialized'):
            self.client = hvac.Client(url=self.url, token='')
    ...
    def pull_data(self, **kwargs):
        return self.client.secrets.kv.v2.read_secret_version(path=kwargs['path'], version=kwargs.get('version'), mount_point=kwargs['mount'])
